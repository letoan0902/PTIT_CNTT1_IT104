...
class Dog extends Pet {
    ...
    getSpecialCareInstructions(): string[] {
        return ["Dắt đi dạo hằng ngày", "Tiêm phòng định kỳ"];
    }
...
class PetShop {
    ...
    addCustomer(name: string, email: string, phone: string): Customer {
        // ID tự động tăng dựa trên số lượng hiện có
    ...
    listAvailablePets(): void {
        ...
        if (availablePets.length === 0) {
            alert("Không có thú cưng nào còn hàng!");
            return;
        }
    ...
    listCustomerPurchases(customerId: number): void {
        ...
        if (customerPurchases.length === 0) {
            alert("Khách hàng chưa có giao dịch nào!");
            return;
        }
    ...
    searchCustomerById(id: number): void {
        ...
        if (!customer) {
            alert(`Không tìm thấy khách hàng với ID ${id}`);
            return;
        }
    ...
    searchPetById(id: number): void {
        ...
        if (!pet) {
            alert(`Không tìm thấy thú cưng với ID ${id}`);
            return;
        }
    ...
    searchSaleById(id: number): void {
        ...
        if (!sale) {
            alert(`Không tìm thấy giao dịch với ID ${id}`);
            return;
        }
...
Chọn: `
    ...
    switch (choice) {
        ...
        case "2": {
            const type = prompt("Loại thú cưng (D-Dog/C-Cat/B-Bird): ")?.toUpperCase() || "";
            const price = Number(prompt("Giá bán: "));
            
            if (!type || isNaN(price) || price <= 0) {
                alert("Thông tin thú cưng không hợp lệ!");
            } else {
                // ID tự động tăng dựa trên số lượng hiện có
        ...
        case "4": {
            const petId = Number(prompt("ID thú cưng cần nhập lại hàng: "));
            if (isNaN(petId)) {
        ...
        case "9": {
            const searchType = prompt("Tìm kiếm (C-Customer/P-Pet/S-Sale): ")?.toUpperCase() || "";
